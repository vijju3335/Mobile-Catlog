        ...
        if login_session['user_id'] == brand.user_id:
            if request.method == 'POST':
                ...
                newMo = Model(
                    ...
                    connectivity=request.form['conn'],
        ...
        if login_session['user_id'] == brand.user_id:
            if request.method == 'POST':
                ...
                return render_template(
                    ...
                    model=editMo, cur_user=login_session)
        else:
            '''flash message'''
            flash("Permissions deniend !")
            return redirect('/')
...
def deleteModel(brand_id, model_id):
    if 'username' in login_session:
        ...
        brand = session.query(Brand).filter_by(id=brand_id).one()
        if login_session['user_id'] == brand.user_id:
            deleteMo = session.query(Model).filter_by(
            ...
            if request.method == 'POST':
                ...
                return render_template(
                    ...
                    model=deleteMo, cur_user=login_session)
        else:
            '''flash message'''
            flash("Permissions deniend !")
            return redirect('/')
...
def showSpecificationsOfModel(brand_id, model_id):
    session = DBSession()
    brand = session.query(Brand).filter_by(id=brand_id).one()
    details = session.query(Model).filter_by(
